...
import { v4 } from "uuid";
import { load } from "./sqlite-vec.js";
	...
	implements IDatabaseCacheAdapter
{
	...
	async createMemory(memory: Memory, tableName: string): Promise<void> {
		...
		this.db.prepare(sql).run(
			...
			tableName,
			content,
			new Float32Array(memory.embedding as any), // Store as Float32Array
			memory.userId,
			memory.roomId,
			memory.agentId,
			isUnique ? 1 : 0,
	...
	async getMemories(params: {
		...
		let sql =
			"SELECT * FROM memories WHERE type = ? AND agentId = ? AND roomId = ?";
	...
	async countMemories(
		...
		let sql =
			"SELECT COUNT(*) as count FROM memories WHERE type = ? AND roomId = ?";
	...
	async createRoom(roomId?: UUID): Promise<UUID> {
		...
		try {
			...
			this.db.prepare(sql).run(id ?? (v4() as UUID));
		} catch (error) {
